...
if(isset($_FILES['upload']) && strlen($_FILES['upload']['name']) > 1) { 
    ...
    if($re == ''){ 
        if(move_uploaded_file($_FILES['upload']['tmp_name'], $uploadpath)) { 
            ...
            $msg = F_NAME .'.'. $type .' อัพโหลดรูปสำเร็จ \\n- ขนาด: '. number_format($_FILES['upload']['size']/1024, 2, '.', '') .' KB'; 
            ...
            $re = '<script>alert("ไม่สามารถอัปโหลดไฟล์")</script>'; 
        } 
    }else{ 
        $re = '<script>
        swal({
            ...
            type: \'success\',
            html: \'<span style="color:gray">'. $re .'</span>\',
